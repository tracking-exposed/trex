...
function hash (obj, fields) {
    ...
    if (print) { debug('(note) hashing of %s', plaincnt); }
...
function verifyRequestSignature (req) {
    ...
    if (req.headers['content-type'] === 'application/json') { message = JSON.stringify(req.body); }
...
function getInt (req, what, def) {
    ...
    if (_.isNaN(rv)) {
        if (!_.isUndefined(def)) { return def; } else {
