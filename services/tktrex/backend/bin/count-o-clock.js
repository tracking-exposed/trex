...
async function start() {
  ...
  debug(
    'Loaded %d possible statistics%s: %d to be done',
    _.size(statsMap),
    name ? `, demanded '${name}'` : '',
    _.size(tobedone)
  );
  ...
  for (const statinfo of tobedone) {
    ...
    const r = await mongo.upsertOne(
      mongoc,
      schema.stats,
      { hourId: hoursref.hourId, name: statinfo.name },
      entry
    );
    hourly.push(r);
  }
