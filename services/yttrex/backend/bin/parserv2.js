...
let skipCount = _.parseInt(nconf.get('skip'))
  ? _.parseInt(nconf.get('skip'))
  : 0;
...
const stop = _.parseInt(nconf.get('stop'))
  ? _.parseInt(nconf.get('stop')) + skipCount
  : 0;
...
const filter = nconf.get('filter')
  ? JSON.parse(fs.readFileSync(nconf.get('filter')))
  : null;
const singleUse = !!id;
...
if (backInTime !== BACKINTIMEDEFAULT) {
  const humanized = moment
    .duration(moment().subtract(backInTime, 'minutes') - moment())
    .humanize();
  debug(
    `Considering ${backInTime} minutes (${humanized}), as override the standard ${BACKINTIMEDEFAULT} minutes ${lastExecution}`
  );
}

function parserDispatcher(envelop, curi, htmlentry) {
  if (htmlentry.nature.type === 'home') return homeparser.process(envelop);
...
function processEachHTML(htmlentry) {
  ...
  try {
    debug(
      '#%d\ton (%d minutes ago) %s %s %s <%s>',
      processedCounter,
      _.round(
        moment.duration(moment() - moment(htmlentry.savingTime)).asMinutes(),
        0
      ),
      htmlentry.metadataId,
      htmlentry.href.replace(/https:\/\//, ''),
      htmlentry.html.length,
      htmlentry.selector
        ? 'S ' + htmlentry.selector
        : 'T ' + htmlentry.nature.type
    );
    processedCounter++;
    ...
    /* ✔️ MODERN, selectors aren't discussed here ✔️ */ else if (
      htmlentry.nature &&
      htmlentry.nature.type
    )
      metadata = parserDispatcher(envelop, curi, htmlentry);
    else debug('Format and versioning bug! %s', htmlentry.metadataId);
...
async function newLoop(htmlFilter) {
  ...
  if (!_.size(htmls.content)) {
    ...
    if (nodatacounter % 10 === 1) {
      debug(
        '%d no data at the last query: %j %j',
        nodatacounter,
        _.keys(htmlFilter),
  ...
  if (!htmls.overflow) {
    ...
    overflowReport(
      ...
      _.first(htmls.content).savingTime,
      _.size(htmls.content),
      _.round(
        moment
          .duration(
            moment(_.last(htmls.content).savingTime) -
              moment(_.first(htmls.content).savingTime)
          )
          .asMinutes(),
        1
      ),
  ...
  if (stats.currentamount || stats.lastamount)
    debug(
      '[+] %d start a new cicle, %d took: %s and now process %d htmls',
      processedCounter,
      stats.currentamount,
  ...
  debug(
    '%d html.content, %d analysis, compacted %d, effects: %j',
    _.size(htmls.content),
    _.size(analysis),
    _.size(_.compact(analysis)),
    _.countBy(updates, 'what')
  );

  /* reset no-data-counter if data has been sucessfully processed */
  if (_.size(_.compact(analysis))) nodatacounter = 0;
  ...
  debug(
    'Usable HTMLs %d/%d - marking as processed the useless %d HTMLs\t\t(sleep %d)',
    _.size(_.compact(analysis)),
    _.size(htmls.content),
    _.size(remaining),
    computedFrequency
  ...
  debug(
    '%d completed, took %d secs = %d mins',
    processedCounter,
    moment.duration(moment() - stats.current).asSeconds(),
...
async function wrapperLoop() {
  ...
  while (true) {
    try {
      ...
      if (type) htmlFilter['nature.type'] = type;

      if (filter) htmlFilter.metadataId = { $in: filter };
      if (id) {
      ...
      if (
        _.size(longlabel.unrecognized) &&
        _.size(longlabel.unrecognized) > lastErrorAmount
      ) {
        const tmpr = await appendLabelError(
          longlabel.unrecognized,
          lastErrorAmount
        );
        debug(
          'Appended last errors in longlabel: %s (%j)',
          _.last(longlabel.unrecognized),
          tmpr
        );
