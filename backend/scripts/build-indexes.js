...
ret = db.leaves.createIndex({ metadataId: 1 }); checkret('leavers metadataId', ret);
ret = db.leaves.createIndex({ savingTime: -1}, { expireAfterSeconds: 7 * 24 * 3600 }); checkret('leaves savingTime expiring', ret);
ret = db.leaves.createIndex({ selectorName: 1}); checkret('leaves selectorName', ret);
