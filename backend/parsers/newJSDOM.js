...
function labelParser(l) {
    try {
        ...
        debug("failure in parsing [%s]: %s", l, error);
        return null;
    }
};

function relatedMetadata(e) {
...
function parseVideoPage(metadata, html) {
    ...
    x = D.querySelectorAll('a > h3');
    y = D.querySelectorAll('ytd-compact-video-renderer');
    p = D.querySelectorAll('ytd-compact-promoted-video-renderer');

    var relateds = _.map(y, relatedMetadata);
