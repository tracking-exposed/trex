...
/*
 *
 * mongodb="mongodb://10.0.2.2/yttrex " KEYS="interactions-interactions,rmap" DEBUG=* since=2018-03-01 mongo-scripts/parserctomy.js 
 *                                            ^^^^^^^^^^^^ ^^^^^^^^^^^^^^^^^
...
function cleanABlock(shift) {
    ...
    return mongo
        .read(nconf.get('schema').videos, selector)
        .map(function(he) {
        ...
        .then(function(htelems) {
            if(_.size(htelems)) {
                ...
                return mongo
                    .updateMany(nconf.get('schema').videos, htelems);
            }
        })
...
function importKeys(csStr) {
    ...
    var f = csStr.split('-');
    var alls = f[1].split(',');
    debug("Results: all the `videos` matching %s will have %s removed", f[0], alls);
    return {
        discrimK: f[0],
        tbremK: alls
    };
