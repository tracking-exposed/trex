...
import { chain, isRight, left, right, Either } from 'fp-ts/lib/Either';
...
export const findElt =
  (selector: string) =>
    (node: SearchableNode): Either<ParseError, SearchableNode> => {
      ...
      if (!elt) {
        return left(
          new ParseError(`could not find element with selector "${selector}"`),
        );
      }
...
export const findAllElts =
  (selector: string) =>
    (node: SearchableNode): Either<ParseError, Element[]> =>
      right([...Array.from(node.querySelectorAll(selector))]);

export const findEltText =
  (selector: string) =>
...
export type Parser<V extends ScrapedValue = ScrapedValue> = (
  node: SearchableNode
) => ParseResult<V>;

export const liftParser =
  (p: (node: SearchableNode) => Either<ParseError, ScrapedValue>) =>
...
const updateErrorPath = (key: string) => (errs: ParseError[]) =>
  errs.map((err) => {
    if (err.missingFields.length === 0) {
      ...
      err.missingFields = err.missingFields.map((field) => `${key}.${field}`);
    }
...
export const combineParsers =
  (parsers: { [key: string]: Parser }): Parser =>
      ...
      Object.entries(parsers).reduce(
        (
          ...
          if (result.errors.length > 0) {
            return {
              ...acc,
              errors: [...acc.errors, ...updateErrorPath(key)(result.errors)],
            };
          }
...
export const parseEltAttrText = (selector: string, attr: string): Parser =>
  liftParser(findEltAttrText(selector, attr));

export const parseAlways =
  <V extends ScrapedValue>(value: V): Parser =>
    (): ParseResult<V> => ({
      value,
      errors: [],
    });
...
export const mapValue =
  <V extends ScrapedValue, R extends ScrapedValue>(fn: (v: V) => R) =>
    (parser: Parser<V>) =>
