...
async function statistics(req) {
  ...
  if (!cache.validSubject(name)) {
    debug(
      'Error! this might not appear in visualization: investigate on why an invalid stat-name is called by c3! (%s)',
      name
    );
    return {
      json: {
  ...
  if (allowedRanges.indexOf(unit) === -1) {
    debug(
      'Error! this might not appear in visualization, but the API call has a malformed time-unit!'
    );
    return {
      json: {
        error: true,
        expectedFormat,
        allowedRanges,
        note: `the statistic unit you look for was ${unit}`,
      },
    };
  }
  ...
  if (_.isNaN(amount)) {
    debug(
      'Error! this might not appear in visualization, but the API call has an invalid number!'
    );
    return {
      json: { error: true, expectedFormat, invalidNumber: req.params.amount },
    };
  }
  ...
  const content = _.map(fullc, function (e) {
    ...
    _.each(_.keys(e), function (k) {
      if (e[k] === 0 || k === '_id') _.unset(e, k);
    });
    return e;
  });

  debug(
    'Requested [%s] since %d %s ago = %d measures',
    name,
    amount,
    unit,
    _.size(content)
