...
function shuffleIf(list) {
  if (!nconf.get('shuffle')) return list;
...
function returnVideosCSVinputs(input, amount) {
  const videos = shuffleIf(
    _.flatten(
      _.map(input, function (smeta) {
...
async function main(): Promise<void> {
  ...
  if (!mongoc) return console.log('Impossible to open a mongodb connection');
  ...
  const final = _.map(
    _.concat(searches[0], homes[0], videos[0], searches[1], homes[1], videos[1])
  );
