...
async function getCampaignQuery(
  campaignColumn,
  queriesColumn,
  campaignName,
  optionalFilter
  ...
  try {
    ...
    debug(
      'getCampaignQuery - campaign retrieved %s, with %d queries',
      campaign.name,
      _.size(campaign.queries)
    );
    const filter = {
    ...
    if (optionalFilter)
      debug(
        'experimental feature for Dot format: %j',
        _.extend(filter, optionalFilter)
      );

    const results = await mongo3.readLimit(
      mongoc,
      queriesColumn,
      optionalFilter ? _.extend(filter, optionalFilter) : filter,
      {},
      MAXAMOUNT,
      0
    );
    const refined = _.map(
      _.groupBy(results, 'searchTerms'),
      function (qlist, searchTerms) {
    ...
    debug(
      'getCampaingQuery collected %d term queries, from non-unique source of %d totals %j',
      _.size(refined),
      _.size(results),
      _.map(refined, 'total')
    );
    ...
    debug(
      'getCampaignQuery: failure in db access (getCampaignQuery %s): %s',
      campaignName,
      error.message
    );
...
async function writeCampaigns(cName, listof, kname) {
  ...
  for (const o of listof) {
    ...
    if (r.insertedCount) debugline += ' inserted';
    if (r.upsertedCount) debugline += ' upserted';
    if (r.matchedCount) debugline += ' matched';
    if (r.modifiedCount) debugline += ' modified';
...
async function getLimitedCollection(cName, filter, maxAmount, reportOverflow) {
  ...
  try {
    ...
    if (reportOverflow && _.size(results) === maxAmount) {
      ...
      debug(
        'the maxAmount for this filter is set to %d. full available %d, sorting by most recent',
        maxAmount,
        full
      );
    }
