import { ParserFn } from '@shared/providers/parser';
import fs from 'fs';
import { HTMLSource } from '../source';
import _ from 'lodash';
import fetch from 'node-fetch';
import path from 'path';
import D from 'debug';
import { YTParserConfig } from '../config';

const debug = D('parsers:thumbnail');
/* if(!nconf.get('thumbnails')) {
     console.log("WRONG CONFIGURATION SETTINGS!! missing 'downloads' from", cwd);
     process.exit(1);
   } 
   'thumbnails' is a git forced directory and it is not created, but 
                it can/should be a nconf                     */

function getUUID(videoId: string): string {
  /* imported from tiktok parsers, shared file, then refactor */
  const filename = `${videoId}.jpeg`;
  const cwd = process.cwd();
  const initials = videoId.substr(0, 2);
  const destdir = path.join(cwd, 'thumbnails', initials);
  if (!fs.existsSync(destdir)) {
    try {
      fs.mkdirSync(destdir, { recursive: true });
    } catch (error) {
      debug('mkdirSync error in %s: %s', destdir, error.message);
    }
  }
  const fullpath = path.join('thumbnails', videoId.substr(0, 2), filename);
  return path.relative(cwd, fullpath);
}

async function downloadIfNew(videoId: string): Promise<{
  videoId: string;
  downloaded: boolean;
  reason: number;
  filename: string;
}> {
  const filename = getUUID(videoId);
  if (fs.existsSync(filename))
    return { videoId, downloaded: true, reason: 0, filename };

  const url = `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
  const x = await fetch(url);
  if (x.status !== 200) {
    debug('Unacceptable HTTP status %d', x.status);
    return {
      videoId,
      downloaded: false,
      reason: x.status,
      filename,
    };
  }
  const data = await x.buffer();
  fs.writeFileSync(filename, data);
  debug('Status 200: written file %s!', filename);
  return {
    videoId,
    downloaded: true,
    reason: 200,
    filename,
  };
}

async function downloadFromVideoId(videoIds: string[]): Promise<any[]> {
  const retval: any[] = [];
  for (const videoId of videoIds) {
    const ret = await downloadIfNew(videoId);
    retval.push(ret);
  }
  return retval;
}

const conditionalDownload: ParserFn<HTMLSource, any, YTParserConfig> = async (
  { html: entry },
  prev,
  config
) => {
  if (config.downloads) {
    debug('Thumbnail disabled by nconf variable!');
    return null;
  }

  let result: any = null;

  if (entry[1]?.experiment?.experimentId) {
    if (entry[1].type === 'search') {
      result = await downloadFromVideoId(_.map(entry[1].results, 'videoId'));
    } else if (entry[1].type === 'video') {
      result = await downloadFromVideoId(_.map(entry[1].related, 'videoId'));
    } else if (entry[1].type === 'home') {
      result = await downloadFromVideoId(_.map(entry[1].selected, 'videoId'));
    } else return [];
  }

  return result;
};

export default conditionalDownload;
