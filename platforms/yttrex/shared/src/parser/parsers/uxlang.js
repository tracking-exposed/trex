...
function getFormatCleanString(publicationString) {
  ...
  const trimmedString = _.reduce(
    absoluteDateIntroSentence,
  ...
  const cleanString = _.reduce(
    conditionalExtra,
    function (memo, o) {
      const chk = new RegExp(o.match).exec(memo);
      if (chk) debug('<!> %s match with: |%s|', memo, o.name);
      return chk ? o.trimWith(memo) : memo;
    },
    trimmedString
  );

  const fitting = _.reduce(
    formatMatches,
    function (memo, re, fmt) {
  ...
  if (!fitting && _.size(cleanString) === _.size(publicationString))
    debug(
      'getFormatCleanString no trim and not match for %s',
      publicationString
    );
  else if (!fitting)
...
function findRelative(stri, clientTime) {
  ...
  try {
    ...
    nlpdebug(
      'Relative match consider %s minus %d %s',
      clientTime,
      amount,
      unit
    );
    ...
    nlpdebug(
      'Relative ERROR: (might happen for many reasons): %s: from |%s|',
      e.message,
      stri
    );
    return moment('invalid date');
...
function findLanguage(type, chunks) {
  ...
  if (!found) {
    debuge(
      'findLanguage failure please add manually: %s |%s|\n%s',
      type,
      _.first(chunks),
...
function sequenceForPublicationTime(D, blang, clientTime) {
  ...
  const m = _.uniq(
    _.compact(
      _.map(D.querySelectorAll('button'), function (e) {
...
*/
  publicationString = D.querySelector(
  ...
  if (publicationString.length > 2) {
    ...
    if (!fmt) {
      nlpdebug(
        "Special management, 'relative timing!' (%s) %s with clientTime %s",
        blang,
        publicationString,
        clientTime
      );
    ...
    if (!mobj.isValid()) {
      debug(
        'Fatal error in deducing format and extrating publicationTime from |%s| with %s\n%j do some googleHappinessCheck ;)',
        publicationString,
        blang,
        moment.monthsShort()
      );
    ...
    nlpdebug(
      'publicationString parsing complete %s\t=>\t%s',
      publicationString,
      mobj.format()
    );
